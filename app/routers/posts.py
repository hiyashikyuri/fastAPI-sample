...
async def create(
        ...
        current_user: User = Depends(get_current_user)):
    return posts.save(db=db, user_id=current_user.id,
...
async def update(
        post: PostUpdate, db: Session = Depends(get_db),
        current_user: User = Depends(get_current_user)):
    return posts.update(db=db, post_id=post.id, title=post.title,
                        body=post.body, user_id=current_user.id)
